...
func Seq2EachKey[K, V any](vr Validator[K]) Validator[iter.Seq2[K, V]] {
	return WithStringFunc(func() string { return fmt.Sprintf("Seq2EachKey(%v)", vr) }, seq2Each(field("key", KeyValue[K, V].GetKey, vr.Validate)))
}
...
func Seq2EachValue[K, V any](vr Validator[V]) Validator[iter.Seq2[K, V]] {
	return WithStringFunc(func() string { return fmt.Sprintf("Seq2EachValue(%v)", vr) }, seq2Each(field("value", KeyValue[K, V].GetValue, vr.Validate)))
}
